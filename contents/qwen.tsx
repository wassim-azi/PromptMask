...
const ProtectButton = () => {
  ...
  const parentClasses = useMemo(() => getParentClasses(appTheme, "qwen"), [appTheme]);
  const buttonClasses = useMemo(() => getButtonClasses(appTheme, "qwen"), [appTheme]);
  const spanClasses = useMemo(() => getSpanClasses(appTheme, "qwen"), [appTheme]);
...
const QwenButtonInjector = () => {
  // MutationObserver for dynamic DOM insertion in Qwen's complex UI
  useEffect(() => {
    ...
    const observer = new MutationObserver((_mutations, obs) => {
      // Look for the container with the class "operationBtn" within chat.qwen.ai's chat box.
      const operationBtnContainer = document.querySelector(".operationBtn");

      // Inject our buttons only if the container hasn’t already been injected.
      if (operationBtnContainer && !operationBtnContainer.querySelector(".protect-button")) {
        const container = document.createElement("div");
        container.className = "protect-button chat-input-feature-tooltip ml-auto";

        // Append our container to the operationBtnContainer.
        operationBtnContainer.appendChild(container);
...
export default QwenButtonInjector;
